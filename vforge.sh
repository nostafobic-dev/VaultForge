...
CONFIG_FILE="${HOME}/.vforge.conf"   # Persistent config file path
...
red="\033[1;31m"   ; purple="\033[1;35m" ; white="\033[1;37m"
dim="\033[2m"      ; reset="\033[0m"  ; magenta= "\x1b[0,32m"
...
_install_termux_systemwide() {
    ...
    echo -e "  ${dim}Copy vforge to \$PREFIX/bin so you can launch it from any folder.${reset}"
    echo -e "  ${dim}Target: ${_install_target}${reset}"
    gap
    echo -e "  ${blue}[1]${reset}  Install now   ${dim}(enables: vforge)${reset}"
            ...
            1)
                if cp "$_self" "$_install_target" 2>/dev/null && \
                   chmod +x "$_install_target" 2>/dev/null; then
                    ...
                    msg info "From now on, type 'vforge' from any folder to launch VIDEO VAULT."
                else
...
is_video "$src" || { msg err "Not a valid video."; exit 1; }
        ...
        case "$_opc" in
            1)
                if [[ -z "$DEFAULT_OUTPUT_DIR" ]]; then
                    msg err "Default output folder is not set — choose another option."
                else
    ...
    read -rp "  ➤ Save extracted payload as (filename only): " _raw; _fn=$(trim "$_raw")
    if [[ -z "$_fn" ]]; then
...
if $USE_FIGLET; then
    clear
    echo -e "${cyan}"
    figlet -f small " VF EXTRACTOR"
    echo -e "${magenta}🌐Github: https://github.com/nostafobic-dev/VaultForge ${reset}"
else
    clear
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║          VF    E X T R A C T O R           ║"
    echo "  ║      Hide & Extract Files Inside Videos       ║"
    echo "  ╚═══════════════════════════════════════════════╝"
    echo -e "${magenta}🌐Github: https://github.com/nostafobic-dev/VaultForge${reset}"
fi
    ...
    echo -e "  ${dim}${red}If handled recklessly, the output file could be currupted.${reset}"
    gap;divider
    ...
    (( _payload_est > 0 )) && check_space "$_od" "$_payload_est"
...
# ── Extract ───────────────────────────────────────────────────────
    ...
    if $ENC_USED; then
        ...
        echo -e "  ${dim}${red} Otherwise, file won't be decrypted even if the passohrase is correct.${reset}"
        gap; divider
        echo -e "  ${dim}  Enter the offset key shown when the file was hidden.${reset}"
    ...
    while true; do
        ...
        if kill -0 $pid 2>/dev/null; then
            target_progress=$(awk "BEGIN {
                p = ($elapsed / $TARGET_TIME) * 99
                if (p > 99) p = 99
                printf \"%.0f\", p
            }")
        else
        ...
        if [ "$DISPLAY_PROGRESS" -lt "$target_progress" ]; then
        ...
        printf "\r  ${green}%s${reset}${dim}%s${reset} %3d%%" \
            "$bar" "$space" "$progress"
    ...
    if $ENC_USED; then
        ...
        while true; do
            _dec_now=$(date +%s.%N)
            _dec_el=$(awk "BEGIN {print $_dec_now - $_dec_st}")

            if kill -0 $_dec_pid 2>/dev/null; then
                _dec_tp=$(awk "BEGIN {
                    p = ($_dec_el / $TARGET_TIME) * 99
                    if (p > 99) p = 99
                    printf \"%.0f\", p
                }")
            else
            ...
            [ "$_dec_dp" -lt "$_dec_tp" ] && _dec_dp=$((_dec_dp + 1))

            _dec_f=$((_dec_dp * BAR_WIDTH / 100))
            _dec_e=$((BAR_WIDTH - _dec_f))
            _dec_b=$(printf "█%.0s" $(seq 1 $_dec_f))
            _dec_s=$(printf "░%.0s" $(seq 1 $_dec_e))

            printf "\r  ${blue}%s${reset}${dim}%s${reset} %3d%%" \
                "$_dec_b" "$_dec_s" "$_dec_dp"

            [ "$_dec_dp" -ge 100 ] && break
            sleep $DELAY
        done
...
if $USE_FIGLET; then
    clear
    echo -e "${cyan}"
    figlet -f small "   VF HIDER"
    echo -e "🌐${magenta}Github: github.com/nostafobic-dev/VaultForge ${reset}"
fi
...
# ── MKV/WebM warning ──────────────────────────────────────────────────
# MKV/WebM parsers stop at the EBML end so appended data is usually ignored,
# but strict parsers may reject the file. MP4/MOV is the safer carrier format.
if file --mime-type "$video" 2>/dev/null | grep -qE "video/(x-matroska|webm)"; then
    gap
    msg warn2 "MKV/WebM detected — some strict parsers may reject files with appended data."
    msg warn2 "MP4 or MOV is the safer carrier format."
    read -rp "  Continue anyway? (Y/n): " _raw
    [[ "${_raw,,}" == "n" ]] && { msg info "Cancelled."; exit 0; }
fi
...
if $ENCRYPT_PAYLOAD && $USE_OPENSSL; then
    ...
    if [[ -z "$_p1" ]]; then
        ...
        TEMP_ENC="$(mktemp "${VAULT_TMPDIR}/vault_enc_XXXXXX.bin")"
        gap
        echo -e "  ${blue}Encrypting payload ⌛︎${reset}"
        gap

        _ENC_BW=30
        _ENC_TT=4
        _ENC_DL=$(awk "BEGIN {print 1/120}")
        ...
        while true; do
            _enc_now=$(date +%s.%N)
            _enc_el=$(awk "BEGIN {print $_enc_now - $_enc_st}")

            if kill -0 $_enc_pid 2>/dev/null; then
                _enc_tp=$(awk "BEGIN {
                    p = ($_enc_el / $_ENC_TT) * 99
                    if (p > 99) p = 99
                    printf \"%.0f\", p
                }")
            else
            ...
            [ "$_enc_dp" -lt "$_enc_tp" ] && _enc_dp=$((_enc_dp + 1))

            _enc_f=$((_enc_dp * _ENC_BW / 100))
            _enc_e=$((_ENC_BW - _enc_f))
            _enc_b=$(printf "█%.0s" $(seq 1 $_enc_f))
            _enc_s=$(printf "░%.0s" $(seq 1 $_enc_e))

            printf "\r  ${blue}%s${reset}${dim}%s${reset} %3d%%" \
                "$_enc_b" "$_enc_s" "$_enc_dp"

            [ "$_enc_dp" -ge 100 ] && break
            sleep $_ENC_DL
        done
        ...
        if [ $? -ne 0 ]; then
            printf "\n"
            msg err "Encryption failed."; exit 1
        fi
        printf "\n"
        secret="$TEMP_ENC"
        ENCRYPT_ACTIVE=true
...
read -rp "  ➤ Name the output video (filename only): " _raw; _fn=$(trim "$_raw")
_ext="${video##*.}"
if [[ -z "$_fn" ]]; then
    output="${_od}/VaultForge_$(date +%s).${_ext}"
...
if $USE_FIGLET; then
    echo -e "${cyan}"
    figlet -f small " VF INJECTOR "
    echo -e "${magenta}🌐Github: https://github.com/nostafobic-dev/VaultForge${reset}"
else
    clear
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║            I N J E C T O R             ║"
